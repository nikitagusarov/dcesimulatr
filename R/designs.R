...
full_factorial_design <- function(experimental_design, n, sample = FALSE) {
  # Avoid check failure
  CID <- i <- NULL
...
##########################################
# 2. Defining "partial factorail" design #
##########################################
...
random_factorial_design <- function(experimental_design, n, sample = FALSE) {
  # Avoid check failure
  CID <- i <- NULL
...
mixed_design <- function(experimental_design, n, sample = FALSE) {
  ...
  if (is.null(Zf) & is.null(Zr)) {
    stop("The resulting Z is empty.")
  }
  if (is.null(Zf)) {
    return(Zr)
  }
