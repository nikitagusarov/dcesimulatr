...
preference_coef <- setRefClass("Preference coefficients",
  fields = list(N = "numeric", p = "numeric", q = "numeric"),
  methods = list(
    gen = function(law = "normal", n = N, m = p, heterogeneity = FALSE, param = list()) {
      if (heterogeneity) {
        generation(law, n, m, param)
      } else {
        matrix(generation(law, n = 1, m, param), ncol = m, nrow = n, byrow = TRUE)
      }
    }
  )
)
