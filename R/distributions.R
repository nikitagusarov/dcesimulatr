...
gen_normal <- function(n, m, param){  
  ...
  if(is.positive.definite(param$sd) & is.symmetric.matrix(param$sd)){
    #the matrix whose lines characterizes every decision makers
    return(rmvnorm(n = n, as.vector(param$mu), param$sd))
  } else {
...
gen_student <- function(n, m, param){
  ...
  if(is.positive.definite(param$scale) & is.symmetric.matrix(param$scale)){
    #the matrix whose lines characterizes every decision makers
    return(rmvt(n = n, delta=as.vector(param$location), sigma=param$scale, df=param$df))
  } else {
